...
%% TODO: add contour plots submit to ncbi!


%%%%##########################################################################
<<echo=false,results=hide,cache=TRUE>>=
#execute the perl script found in the present folder
S <- as.data.frame(read.delim(pipe("./pilot.pl"), sep=",", header=FALSE, as.is=TRUE))
#use the rows specified as count in a new dataframe
SS <- S[S$V4=="count",]
SS$V4 <- NULL
names(SS) <- c("lib","sort", "count")
SS$count <- as.numeric(as.character(SS$count))

#use the remaining rows in another
GC <- S[S$V4!="count",]
names(GC) <- c("species","diagnosis","read","gc")
GC$species <- gsub("_total", "",GC$species)
GC$gc <- as.numeric(as.character(GC$gc))
@ 

<<echo=false,results=hide>>=
#converte to wide data format and make nice names
ALL <- reshape(SS,  direction="wide", v.names="count",  timevar="sort",  idvar="lib")
names(ALL) <- gsub("count.","", names(ALL))
row.names(ALL) <- ALL$lib
ALL$lib <- NULL

## resort to look better
col.wanted <- c("short", "poly","rRNA", "fishpep","good2")
row.wanted <- c("Ac_197F","Ac_106F", "Ac_M175",
                 "Ac_FM", "Ac_EH1", "Ac_total", "Aj_Li1", "Aj_Li2", "Aj_Li3", "Aj_total")
ALL <- ALL[row.wanted,]
A <- as.data.frame(ALL[,col.wanted])
...
nlib <- paste(row.names(A), ntotal, sep ="")

# total n in the library description
An <- as.data.frame(A, row.names=nlib)
...
GP <- apply(A[,c("rRNA", "fishpep")],2,gperc)
...
Sanger-technology. In total \Sexpr{ALL["Ac_total","total"]} reads from
adult \textit{A. crassus} (5 libraries from 4 cDNA preparations) and
\Sexpr{ALL["Aj_total","total"]} reads from liver-tissue of the host
species \textit{Anguilla japonica} (3 libraries from 3 cDNA
preparations) were sequenced. \Sexpr{ALL["Ac_total", "good1"]} of the
nematode and \Sexpr{ALL["Aj_total", "good1"]} of the host reads were
...
<< echo = false,results = hide >>= 
tee <- t.test(GC$gc[GC$species == "Ac"], GC$gc[GC$species == "Aj"])
@
...
\Sexpr{A["Ac_total","good2"]} nematode- and
\Sexpr{A["Aj_total","good2"]} host quality ESTs submittable to NCBI
